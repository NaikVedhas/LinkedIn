...
const ProfileHeader = ({ userData, onSave, isOwnProfile }) => {
  ...
  const { mutate: followUser } = useMutation({
    ...
    onSuccess: (response) => {
      //this response is the response that we get from backend we can name it anything
  ...
  const renderConnectionButton = () => {
    const baseClass =
      "text-white py-1 px-4 rounded-full transition duration-300 flex items-center justify-center";
